...
- **Re-render (Tier-B)** when the page's evidence depends on a region the cached shot got *statically* wrong — grey/WebGL hero, black media, lazy-load gaps, unsettled animation — **or** an overlay covers the content. Add `--dismiss` for the overlay case:
  ...
  - **Timed/marketing modals** (newsletter, "10% off") — if there's a dismiss control (Escape, "No thanks", "×"), `--dismiss` may clear it in one shot. If the modal re-arms during the warm-scroll loop (gethealthspan, 2026-06-16 — see [BACKLOG](../../BACKLOG.md)) or has no dismiss path, `scroll_locked` fires; exclude or caveat the tile instead.
