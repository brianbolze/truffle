...
Tier-0 `profile.md` is the **default** — one company → one dossier, no hand-holding. Three opt-in extensions are live: **`offerings.md`** (Tier-1 per-SKU roster — separate recipe in [`firecrawl-capture.md` §1.1](firecrawl-capture.md), **only** when a cohort needs the per-SKU grain, telehealth first; lint `scripts/offeringscheck.py`; deepen an existing roster later with the `/deepen-offerings` preset), the **`logos:{}`** module (multi-ratio brand marks added to `profile.md` — [§1.2](firecrawl-capture.md); near-free, measured by `fc.py logos`), and the **`telehealth.md`** cohort pack (the first **cohort pack** — vertical classification cuts for a telehealth company that the universal profile reads identical; contract [`TELEHEALTH.md`](../../modules/TELEHEALTH.md), lint `scripts/cohortcheck.py --cohort telehealth`).
