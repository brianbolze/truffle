...
**`is_multi_product: false` — and this is the cohort's lone `false`.** Unlike every peer (Eden/Hone/PeterMD/Hims, all multi-vertical), TeloLife sells *one thing*: GLP-1 weight loss. Semaglutide vs tirzepatide are **drug options within the one program** (a clinician picks the molecule for you), and 3/6/9/12-month are **duration variants** — not distinct offerings a customer comparison-shops between. By the TAXONOMIES test (and the Benadryl "one purpose, many forms → false" cue), this is a clean `false`. **It is also a real divergence, not a classification wobble:** TeloLife *used* to be multi-product (the 2026-05-11 snapshot had three verticals) and deliberately dropped longevity + hair to become GLP-1-only.
