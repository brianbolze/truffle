...
**`portfolio_shape: Single` — but the closest-to-the-line `Single` in the corpus.** By the TAXONOMIES test (2+ distinct offerings you'd comparison-shop or buy separately would push toward `Multi-product`), the SKUs are **forms and strengths of one brand**, not separately-branded products — no sub-brand ever splits off (contrast AG1 → AGZ, which earned its own name and tips AG1 to `Flagship + companions`). It reads like **linear's "one thing, many surfaces" `Single`**, just expressed as delivery forms instead of software modules. *What pulls it toward the multi-product end:* an oral allergy tablet and a topical itch cream are genuinely different product types for different occasions — a shopper buys the one matching the symptom, not "a Benadryl." Resolved `Single` because they share one active ingredient and one brand identity and are positioned as a single family ("there's a BENADRYL® for you"). This "one purpose, many forms" pattern is a **distinct flavor of `Single`** — see FINDINGS.
