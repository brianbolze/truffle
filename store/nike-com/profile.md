...
**`portfolio_shape: Catalog` — the scale extreme.** Thousands of distinct, separately-bought products across dozens of franchises and sports; you absolutely comparison-shop Air Max vs. Pegasus vs. Jordan. The corpus lesson: `portfolio_shape` is **trivial at the extremes** (Nike/AWS obviously `Catalog`; a one-product SaaS obviously `Single`) and **only hard in the middle** — the single-brand-many-forms cases like Benadryl (`Single`) and the flagship-plus-companions case like AG1 (`Flagship + companions`), which are exactly why the ordinal needs those two middle values. Nike anchors the "huge `Catalog`" end.
