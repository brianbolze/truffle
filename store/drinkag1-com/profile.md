...
**`portfolio_shape: Flagship + companions` — but genuinely debatable.** By the TAXONOMIES test (2+ distinct offerings a customer would comparison-shop or buy separately, distinct in name/page/positioning), AG1 / AGZ / D3+K2 / Omega-3 clearly clear the `Single` bar — each has its own PDP and standalone purchase. *What holds it back from full `Multi-product`:* the brand explicitly self-positions as anti-proliferation — *"focus on a very small number of products… we never make anything new just to sell you on more"* — and AG1 still carries ~all the brand equity, with the others a small companion stack around it. This sits one notch up from linear (which is `Single`: one app, many surfaces, one price): AG1 is one hero brand plus several separately-bought companions, several prices. The flagship-plus-stack shape is the recurring hard case — and the reason the ordinal carries a distinct `Flagship + companions` value between `Single` and `Multi-product`.
