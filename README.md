...
scripts/      # engine utilities (querycheck.py — QUERYING.md drift self-test; store.py — resolve()/relations(); offeringscheck.py + cohortcheck.py — module-contract linters; build_db.py — corpus-wide SQLite lens with cohort-gated views, --check-guarded; render.py + compare.py — CLIs over the present/ package, the human-facing lens: briefs, comparison sheets, corpus index)
