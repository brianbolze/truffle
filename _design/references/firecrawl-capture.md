...
**Budget telemetry — free.** `GET /v2/team/credit-usage` returns `remainingCredits` / `planCredits` + the billing window, at **zero credit cost**. The verb should pre-flight it (warn/abort when low) and report spend in its run summary — the cleanest expression of "Firecrawl is the only real money." (Sibling `GET /v2/team/token-usage` tracks the LLM-extraction bucket — relevant only as a signal to keep the extract features we avoid untouched.)
...
- **Don't set `removeBase64Images:true`** — it strips inline data-URI logos (linear's logo was exactly that). Default (keep) is correct.
