...
- **Split like visual-quality.** Graduate the cited, axis-specific **evidence layer**; hold the viewer-relative **"formidable?" verdict** outside the shared store for now — an open edge, not a closed call. *No blend.*
...
- **(Open edge)** — a formidability verdict; depth-gating a capture. Engine supplies the signal; who makes the call is under active rework.
...
- **Grain is the constant trap.** Company review velocity ≠ SKU demand; SERP rank ≠ conversion; roster breadth ≠ traction. v2 makes *"every signal keeps its grain"* rule #1.
...
- **The live consumer draws the engine/project line.** v2 keeps traction as a *project judgment* layer — engine owns generic State + capture tools; the project owns the signal cards, project grain (SKU/molecule), and every judgment — the v2 division today. *"The Web Research tools should stay generic. This layer should not."* So even the evidence layer is project-flavored today; only the generic machinery is a lift candidate.
...
- **Not (today)** the formidability / threat / fit **judgment** — outside the shared store; an open edge under rework.
