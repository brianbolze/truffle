...
> **What the engine owns — and doesn't.** This is the **State / Signals / Judgments** split from the [Frame](2026-05-29-frame.md), made operational. The engine owns **State** — generic, cross-domain **classification** (`entity_type`, `offering_category`, … — see [`TAXONOMIES.md`](../TAXONOMIES.md)) and company-intrinsic facts/observations, captured as a current snapshot. It also owns the **generic Signals machinery** — the reusable capture tools, the comparator, and append-only evidence (where the timeline output lands is still in design). Two kinds of thing land here but are **project-designed**: **market-vertical / cohort cuts** ("Weight Loss," "Hormone Therapy") — the project defines the cut; the engine holds and serves it. What stays **out of the shared store today**: **user-relative judgments** — strategic relevance, importance, competitive threat, fit — which depend on *who's asking*; whether and how the engine ever emits them is an **open edge under active rework**, not a closed no. Rule of thumb: the engine **describes the company and how it's moving**; what that *means* to a given asker stays with the consumer, for now. (The optional `Strategic read` section is company-*intrinsic* observation — "what's notable here" — not relevance-to-you.) Market position rides the same line: record what a site foregrounds as observation, but inferring flagship status / market leadership / adoption from prominence is a judgment about *the market*, not the company — a consumer-layer call, deferred (a future Frame session, not built into capture).
...
> **The store holds snapshots today.** The **Signals** layer (funding, traffic-over-time, review/visibility movement) is a *different record shape* — dated, append-only — and is **not in the store yet**. `tools/` now provides reusable source captures for some signals, but not a durable timeline. *Where web-research's domain-keyed signal output lands is still in design* — the deliberate call worth resolving before the aggregation layer hardens. The [traction frame](2026-06-14-traction-frame.md) now frames this decision (Open Qs #1, #6).
...
- **Signals** — the same facts on a time axis (funding, traffic, headcount; review deltas; SERP movement; news/M&A as dated events) → the **engine's reusable capture tools + comparator** today; where the durable timeline lands is still in design; never the profile.
