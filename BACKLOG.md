...
- **`offerings.md` completeness is prose-buried — a roster count can't be trusted as catalog breadth** `[tbd]` `[md]` `[@brian]`
  An `offerings.md` deliberately stops at "complete at the indexed level" to spare Firecrawl credits, and records *how far it got* only as **prose in `## Provenance`** — inconsistently, and not machine-readable. So a count (`sku_count`, or an eyeballed roster length) **conflates "small catalog" with "capture stopped early."** The [2026-06-04 sqlite-aggregation probe](experiments/2026-06-04-sqlite-aggregation/) hit this live: **Ro.co sorted dead-last (8 SKUs)** off a weight-loss-only run while actually carrying ~36 (a re-capture fixed it); the cohort audit then found the verdict prose-buried + inconsistent — a keyword triage false-flagged **5 of 8** candidates (a benign "couldn't reach all-in cost" reads identical to a real "whole line out of scope").
  **Decide:** promote the prose verdict into a structured `offerings.md` frontmatter cut so a count is never read naked — e.g. `enumeration: complete-at-indexed-level | partial-lines-omitted | flagship-only | catalog-exemplars` (replaces the Provenance verdict's load-bearing role; the prose detail stays as colour). Open qs (the "get it right"): **(a) granularity** — one company-level value flattens the real shape (honehealth: Rx lines complete *and* the `shop.` supplements subdomain out of scope = per-*line* scope, not per-company); **(b) vs `portfolio_shape`** — a `Catalog` is *expected* incomplete-at-leaf, so the cut likely means "did we reach the indexed level, or fall short," not "is every SKU here"; **(c)** keep it **one** closed-set field — resist a sub-schema.
  NOTE: May want to revisit the `/deepen-offerings` skill once we make changes.
  **Act when:** a consumer leans on roster counts/aggregates (e.g. the parked rung-3 index graduates), or a 2nd count-trust miss lands (Ro.co is 1). Until then the probe's [`CAVEATS.md`](experiments/2026-06-04-sqlite-aggregation/CAVEATS.md) "counts are a floor" guard holds the line.
