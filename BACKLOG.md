...
- **Credit accounting is unreliable** `[weakness]` `[sm]`
...
- **Underused capture payload — scoped signal audit** `[idea]` `[md]`
...
- **Light, safe, honesty-preserving cleaning / normalization pass on the payload markdown** `[idea]`
- **Rung-3 SQLite index — not yet** `[idea]` `[xl]`
