...
- **`log.md` as the Signals-layer shape — storage decided; OKF visualizer still worth watching** `[idea]` `[md]`
...
- **Traction module / verb — v1 shipped; card layer + cohort frame still parked** `[xl]` `[parked]` `[@brian]`
  v1 of the per-company evidence layer shipped 2026-06-15 (the frame's first graduation: axis-specific evidence, no blend, no verdict): `tools/signal_delta.py` (envelope comparator — trustpilot/serpapi/trends/wayback branches; deltas + comparability vetoes; no score by construction) + `tools/sec_edgar.py` (keyless first-party funding — ticker→State, Form-D/filing→Signal, `name_match_unconfirmed` guard), with the `store/<domain>/signals/<source_type>/<captured_at>.json` path committed to the [architecture](_design/2026-05-30-architecture.md). Frame: [`_design/2026-06-14-traction-frame.md`](_design/2026-06-14-traction-frame.md) · approach: [`_design/2026-06-15-traction-approach.md`](_design/2026-06-15-traction-approach.md).
