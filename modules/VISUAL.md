...
- **`qa_status`** (closed set) — the capture-health verdict the QA gate reached: `clean` (all cited tiles came straight from cached payloads, no exclusions) · `exclusions-noted` (≥1 tile excluded for contamination; the `## Provenance` exclusion note names them) · `recapture-used` (Tier-B browser re-render supplied ≥1 page's tiles because the cached Firecrawl screenshot was contaminated — WebGL/lazy/animation **or** an overlay the cached payload still carries; the `## Provenance` note names which pages and, when Tier-B ran with `--dismiss`, names the overlay it cleared). Affordance-only dismissal is a Tier-B sub-mode, not a new status — `shoot.py`'s `manifest.json` records both `dismissed` (did `--dismiss` run) and `dismiss_cleared` (did it *work* — the overlay footprint dropped; `false` = a no-op the operator must caveat/exclude), the airtight which-is-which.
...
- **Tier B — browser re-render (escalation).** When the QA gate flags a contaminated page the cached screenshot can't fix — a WebGL/grey hero, black media cards, lazy-load gaps, mid-animation reveals, or an overlay (cookie banner / newsletter modal) covering content — re-render it in a real browser (`scripts/shoot.py`: Playwright drives system Chrome with real WebGL, warm-scrolls to fire lazy media, reduced-motion + settle, and on `--dismiss` clears overlays via the page's *own* dismiss affordances — Escape + scoped dismiss-button clicks, never a vendor denylist; validated in [`experiments/2026-06-16-tier-b-dismissal/`](../experiments/2026-06-16-tier-b-dismissal/FINDINGS.md)). No Firecrawl spend. Tiles + an `overview-480w.png` (the QA gate's overview) land beside the capture; `qa_status: recapture-used`. **Dual-render only when contamination is combined** — a page broken by *both* WebGL/lazy and an overlay can't use the cached payload as the faithful baseline, so render twice into sibling dirs: `tiles/<page>` (faithful, no flag) + `tiles/<page>__dismissed` (`--dismiss`). Cards cite the `__dismissed` tiles; the faithful set is kept on disk as the comparison view.
