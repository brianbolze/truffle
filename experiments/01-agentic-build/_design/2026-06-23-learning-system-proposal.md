...
- **Observations are immutable — with one exception.** Once written, an observation is never edited or deleted. The *only* permitted touch is a `consumed-by: <lesson-id>` stamp added when it graduates — so a reviewer can see at a glance which raw notes are already accounted for, without rewriting the note.
