...
def do_scrape(url, slug, name, homepage, wait, proxy):
    ...
    print(
        f"[scrape] {name:28} status={status} mdlen={len(md):>6} {dt:4.1f}s "
...
def do_verify(slug):
    ...
    for md5, names in by_md5.items():
        if len(names) > 1:
            ...
            bad = True
    print(
        "  OK — all sourceURLs match, all bodies unique"
        if not bad
        else "  ^^ ISSUES above"
    )
...
if __name__ == "__main__":
    ...
    a = ap.parse_args()
    if a.cmd == "map":
        do_map(a.url, a.slug, a.search, a.limit)
    elif a.cmd == "scrape":
        do_scrape(a.url, a.slug, a.name, a.homepage, a.wait, a.proxy)
    elif a.cmd == "verify":
        do_verify(a.slug)
    elif a.cmd == "credits":
        do_credits()
