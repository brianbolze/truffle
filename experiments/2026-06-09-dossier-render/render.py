...
from store import load as store_load, resolve as store_resolve  # noqa: E402
...
def _read_doc(path: str) -> tuple[dict[str, Any], str]:
    with open(path, encoding="utf-8") as f:
        text = f.read()
    if text.startswith("---"):
...
def _col(header: list[str], *prefixes: str) -> int | None:
...
def extract_offerings(slug: str) -> dict[str, Any] | None:
    ...
    roster_md = sections.get("roster", "")
    table_lines = [ln for ln in roster_md.split("\n") if ln.strip().startswith("|")]
    ...
    if len(table_lines) >= 3:
        ...
        ci = {
            "name": 0,
            "kind": _col(header, "kind"),
            "slug": _col(header, "slug"),
            "price": _col(header, "price"),
            "vis": _col(header, "visibility"),
            "what": _col(header, "what"),
        }
        ...
        for r in rows:
            ...
            if kind == "family":
                cur = {"name": re.sub(r"\*\*", "", r[0]), "what": cell("what"), "skus": []}
                groups.append(cur)
            else:
                buyable += 1
                if cur is None:
                    cur = {"name": "", "what": "", "skus": []}
                    groups.append(cur)
                cur["skus"].append({"name": re.sub(r"\*\*", "", r[0]), "slug": cell("slug"),
                                    "price": cell("price"), "vis": cell("vis"), "what": cell("what")})
    return {
...
def extract_model(query: str, fetch: bool = True) -> dict[str, Any] | None:
    ...
    if not slug:
        return None
    fm, body = _read_doc(os.path.join(STORE, slug, "profile.md"))
...
def main() -> None:
    ...
    if not args:
        sys.exit("usage: render.py <company> [<company>…] [--no-fetch]")
    os.makedirs(OUT, exist_ok=True)
    for q in args:
        m = extract_model(q, fetch=fetch)
        if m is None:
