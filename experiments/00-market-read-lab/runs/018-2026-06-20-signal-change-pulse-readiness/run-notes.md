...
- **What was awkward?** Nothing in execution — the tool's veto-not-skip behavior made the "what can't we diff" answer fall out for free. The substantive friction is in the *store*, not the run: SEC has no delta branch, SERP captures are one-per-query, and only Trustpilot has a real time-gapped second capture.
...
- The other system friction surfaced by the tool: `signal_delta.py` has no `sec_edgar` branch, so SEC pairs return a veto string instead of a diff. Correct fail-closed behavior, but it means SEC change-pulse is hand-work today.
...
| `tooling-ergonomics` | `signal_delta.py` has no `sec_edgar` delta branch; SEC pairs veto instead of diffing. Narrow, concrete tool gap. | watch for recurrence (fold into the same new item as the one buildable fix) |
