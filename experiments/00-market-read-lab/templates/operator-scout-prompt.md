...
4. Update only the YAML header at the top of the target run's `run-notes.md`:
   ...
   - If the selected question needs live external evidence, approval, or human
     reframing, set `run_status: needs-human-review`, use
...
- Prefer unattended-safe candidates: store-only, evidence-ready, no live spend.
- Still include some beyond-store candidates when they expose an important source
  ingredient, membership, relation, or grain gap; mark them `live-external-needs-approval`.
