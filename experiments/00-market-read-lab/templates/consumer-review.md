...
|---|---|---|
| **Useful** | Clear answer, decision aid, or next step; not just a summary. |  |
| **Judgment-ready** | Fresh, rare, cited ingredients a human or downstream system could reason from. |  |
| **Sourced & cited** | Claims trace to dated captures, receipts, or store files; uncertainty is visible. |  |
...
Keep only the jobs this read actually served. Delete rows that do not apply.

| Job | Did the read help? | Missing / next step |
|---|---|---|
| **Make AI safe to delegate to** | Grounded, current inputs an agent can use without quietly inventing facts. |  |
| **Cold-start a company** | Trustworthy cited profile for a company I do not know. |  |
| **Compare a whole field** | Complete, cited answer across a category or cohort. |  |
| **Build on top without re-capturing** | Fresh, trustworthy state/signals a downstream system can query, enrich, monitor, and judge. |  |
| **Trust the cache over time** | Dated change, stale assumption, or monitoring need surfaced. |  |
| **Five-second brief input** | Verbatim language, pricing, whitespace, or market shape a strategist can use fast. |  |

## Lens check

- **Strategist:** does the answer land plainly and quickly? Does it provide novel insights that are hard to get elsewhere?
