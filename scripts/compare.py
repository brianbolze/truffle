#!/usr/bin/env python3
"""compare — N company dossiers → one side-by-side specimen sheet (self-contained HTML).

Why: the brief answers "tell me about X"; this answers the brand-strategist's real question —
"show me X *against its category*." It renders the store's captured states in horizontal lens
bands (identity, voice, color, type, audience, offer posture, field specimens), each cut into
one column per company, so competitor patterns read in a glance.

What it deliberately does NOT do: normalize prices or units across companies — the judgment
BACKLOG parked this view on. Money appears only as *price-visibility posture* (published /
partial / on-request counts, a closed set), which is comparable without a single conversion.
All language is verbatim from the dossiers; nothing is paraphrased at render time.

One script, one output file, no server. Reuses render.extract_model — the brief and the sheet
are two lenses over the same extraction, by design.
"""
...
from render import GRAIN, INK, OUT, PAPER, _hex_rgb, _peek, _truncate, build_fonts, esc, extract_model, load_screenshot  # noqa: E402
