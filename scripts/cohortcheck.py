...
A *cohort pack* (contract: `<COHORT>.md`, e.g. TELEHEALTH.md; species note: SCHEMA → Tier-1 modules) is the
...
def load_contract(cohort: str) -> dict[str, Any]:
    """The cohort's machine-readable spec, sliced from `<COHORT>.md` — `{cohort, doc_meta, fields:{name:[...]}}`.
    ...
    """
    path = os.path.join(ROOT, f"{cohort.upper()}.md")
    if not os.path.isfile(path):
        sys.exit(f"cohortcheck: no contract {cohort.upper()}.md at repo root — is '{cohort}' a real cohort?")
...
def main() -> None:
    ap = argparse.ArgumentParser(description="lint a cohort pack against its <COHORT>.md contract")
    ap.add_argument("--cohort", required=True, help="cohort name, e.g. telehealth (resolves to TELEHEALTH.md)")
