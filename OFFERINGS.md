...
- **`## Roster`** — one row per offering, **complete at the indexed level** (the level the company indexes at, never every leaf). Columns, exactly:
  ...
  - **Price (verbatim)** — quoted exactly, with its footnote markers, or `—` for family rows. Never normalized.
