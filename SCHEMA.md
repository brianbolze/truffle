...
*Module contracts live in `modules/` — two contracts (OFFERINGS + VISUAL) earned the directory. Cohort packs stay **top-level CAPS files** (TELEHEALTH.md at root). The directory name is **not** `cohorts/` — that name is taken by the architecture's store-level `cohorts/<category-slug>/` cross-company *signals* layer ([architecture](_design/2026-05-30-architecture.md)), a different concept (Signals by category, not per-domain State).*
