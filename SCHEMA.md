...
- **`offerings.md`** — the **per-SKU roster** (active, contract settled 2026-06-03). The family-grain `profile.md` *What they offer* line collapses a brand's six GLP-1 SKUs into one bullet; this is the opt-in per-SKU/molecule layer for a cohort whose consumer needs the grain. **Roster-first body**: `## Portfolio overview` (brief + a *calibrated* prominence read) → `## Roster` (the load-bearing table — `Offering · Kind · Parent · Slug · Price (verbatim) · Visibility · What (molecule · form · access)`, one row per offering, complete *at the indexed level*) → `## Deep blocks` (**earned**, not a default top-N — only where a verbatim H1 / price footnote / disambiguation resolves an ambiguity a roster row can't). Molecule rides **inside `What`, page-attested only** ("not stated" if the page is silent) — never a lead column, never a stored cross-company key (grouping is query-time `rg`). Owns `price_visibility` per-SKU when active; the `profile.md` family-line token stays the convention everywhere else. Contract + recipe + verdict: [`_design/2026-06-03-offerings-module.md`](_design/2026-06-03-offerings-module.md); capture recipe in [`firecrawl-capture.md` §1.1](skills/research-company/firecrawl-capture.md); lint with `scripts/offeringscheck.py`. **First enabled set: the telehealth cohort**; default elsewhere stays *don't write the file*.
