...
> **Current contract version: `2.6`.** This is what new captures stamp as `schema_version`. Bump rules + the version history are in the `schema_version` note below (under the frontmatter block).
...
schema_version: "2.6"                # contract MAJOR.MINOR this profile obeys; querycheck warns if it outruns SCHEMA's current version (see note below)

# Identity
domain: honehealth.com               # primary key
name: Hone Health
aliases: []                          # alt names/domains of the SAME entity (rebrand + M&A escape hatch)
legal_entity: ""                     # 2.6 — registered legal name, ONLY when site-derivable (JSON-LD legalName / ©/footer), e.g. "Time Therapeutics, Inc." Empty if the site doesn't state it; never filled by external research. (See relations + structured-layer notes.)
...
logo_url: assets/wordmark.svg        # the single "just give me the logo" mark — new captures (2.5+) canonicalize it to the WORDMARK; older/empty = the favicon fallback
logos:                               # OPTIONAL 2.5 module — multi-ratio mark set; each entry records what was found + ITS measurements (the consumer applies the bar). Absent/empty = predates it. (See Logos note + recipe §1.2.)
  wordmark: { src: assets/wordmark.svg, w: 232, h: 65 }                                                              # rectangle, mark+name — PRIMARY slot, ~always present
  logomark: { src: "https://www.google.com/s2/favicons?domain=honehealth.com&sz=256", px: 256, transparent: true }  # square — best-effort, >=128px short side
  og:       { src: "https://honehealth.com/img/og.png", w: 1200, h: 630 }                                           # wide cover (Notion) — best-effort, >=600px width
...
- ***MINOR*** *(`1.0`→`1.1`) — an **additive**, backward-compatible change: a new optional field, value, or section. **No backfill, no re-stamp** — grandfather the old number, so an empty new field in an older profile reads as "predates the field," not "missing data." When the new field cannot backfill (empty on an old profile = "predates it"), **append `display-name → version` to `FIELD_VERSIONS` in `scripts/store.py`** — the `predates:` computation and `querycheck --strict` depend on that mirror staying current.*
...
- ***2.3*** *(MINOR, additive — no backfill, grandfathered → FIELD_VERSIONS) — added the per-offering **price-visibility token** `` `[published | partial | on-request]` `` on `What they offer` body lines: the one pricing axis that generalizes ("can I even get a price?"), greppable, **wrapping** the verbatim price (never replacing it). Grounded in [Probe 0](experiments/2026-06-01-profile-enrichment/FINDINGS.md) — enriching `profile.md` with verbatim price barely helped a cold consumer, but the visibility token was the durable win (it also defers `offerings.md`; see BACKLOG). Absent token on a pre-2.3 profile = "predates the convention," not "published"; no backfill (a cohort consumer can tag on demand).*
- ***2.4*** *(MINOR, additive — no backfill) — added an optional **`Run profile`** line to the Provenance set: when a **guided** capture deviated from a plain run (an emphasis steered page selection, a module like `offerings.md` was added, or a refresh was forced over a still-warm capture), one line records what changed vs. vanilla (e.g. `Run profile: guided — emphasis "enterprise pricing"; +offerings`). A vanilla run writes nothing — its **absence reads "plain capture," not "missing."** Pairs with the `/research-company` guided pre-flight ([skill step 2.5](skills/research-company/SKILL.md)). No linter/QUERYING change — Provenance is existence-checked, not a closed line set.*
- ***2.5*** *(MINOR, additive — no backfill, grandfathered → FIELD_VERSIONS) — added the optional **`logos:{}`** visual-identity module: a multi-ratio brand-mark set (**wordmark / logomark / og**), each slot recording its own measurements (`w`/`h`/`px`/`transparent`) as **facts the consumer gates on, never pre-dropped** — a small mark is recorded with its size, a slot omitted only on true absence (see the [Logos note](#logos) + recipe [§1.2](skills/research-company/firecrawl-capture.md)). `logo_url` stays; on new captures it **canonicalizes to the wordmark** (old values grandfathered). Assets store as a **stable URL** where one exists, else a committed text SVG under `store/<domain>/assets/`. Empty/absent `logos` on a pre-2.5 profile = "predates the module," not "no logos" — **no backfill, no re-stamp**. De-risked by the [logo probe](experiments/2026-06-03-logo-probe/FINDINGS.md) (per-slot deck-ready: wordmark 16/16 · logomark 13/16 · og 11/16 · union 16/16); design [`_design/2026-06-03-logos.md`](_design/2026-06-03-logos.md).*
- ***2.6*** *(MINOR, additive — no backfill, grandfathered → FIELD_VERSIONS) — promoted **`legal_entity`** out of `aliases`: the registered legal name (JSON-LD `legalName`, ©/footer) becomes its own queryable scalar instead of folding into the alias list, so a consumer can ask "what entity is X?" without guessing which alias is the legal one. **Site-derivable only** — empty when the page doesn't state it, never filled by external research (that would break the site-derived-State line; **founders stay prose-only** for the same reason — the 2.2 call holds). Only `legalName` moves; `alternateName` still → `aliases`. Empty `legal_entity` on a pre-2.6 profile = "predates the field," **no backfill, no re-stamp**. Also sharpened the `parent`/`owns` write rule (affiliation ≠ ownership; see the [relation-evidence note](#relation-evidence)) and turned the existing `logos:{}` measurement contract into an enforced lint (see the [logos-lint note](#logos-lint)) — both clarify existing fields, neither is itself a version event. Capture-trust pass from the [model bakeoff](experiments/2026-06-13-research-company-model-bakeoff/FINDINGS.md).*
...
*<a id="relation-evidence"></a>**Evidence bar — ownership, not affiliation.** `parent`/`owns` require **explicit ownership/legal attestation**: "acquired", "wholly-owned subsidiary", "a [Parent] company", "part of [Parent]", a `©`/legal-footer line naming the owner. Mere **`partner` / `family` / `collective` / `group` / "family of partners"** framing is *affiliation, not ownership* — record it in prose (Overview / Strategic read), and leave the relation field empty. A capture can't see a cap table; suggestive corporate language is not a relation. *(Motivating miss: Red Antler's homepage "a family of best in class partners" was over-read into `owns: [wildfruit.co, fat-earth.com]` — see the [model bakeoff](experiments/2026-06-13-research-company-model-bakeoff/FINDINGS.md). A skeptical second-model claim-audit is the cheap runtime catch; [skill step 7](skills/research-company/SKILL.md).)*
...
*<a id="logos"></a>**Logos (`logos:{}`, 2.5 module — gathered by default at capture).** The multi-ratio mark set a slide / Notion-cover consumer needs beyond `logo_url`'s one mark. Three best-effort slots, each recording **its own measurements as facts, not gates** — the consumer applies the size bar, the engine never pre-drops a mark (a small one is recorded with its measurement, e.g. `logomark: { px: 32 }`; a slot is omitted **only on true absence**). `src` is a **stable URL** where one exists (the google-s2 logomark, the `og:image`, a hostable wordmark) — regenerable, no file; where there's none (an extracted inline-SVG wordmark) it's a **committed text SVG** under `store/<domain>/assets/` (text → commits clean, scales infinitely; a rare raster header-crop with no URL goes to a gitignored sidecar — local-only, so commit the tiny PNG if it must survive a fresh clone). The slots + their source chains (measured, never trusted — full recipe in [`firecrawl-capture.md` §1.2](skills/research-company/firecrawl-capture.md)):*
- ***`wordmark: { src, w, h }`*** *— rectangle, mark+name; the **primary/reliable** slot. Needs the **agent's eyes**: the hostable `logo_url` if it's a real wordmark, else extract the inline `<svg>` — a blind image scan grabs press logos (Probe 2). `logo_url` canonicalizes to it on new captures.*
- ***`logomark: { src, px, transparent }`*** *— square; deterministic: the **larger measured** of google `s2/favicons?...&sz=256` and the apple-touch-icon (`sz=256` is a request, not a promise — measure the bytes; `px` is the short side). `transparent` is the agent's call on a checker tile (`hasAlpha` lies — `false` = a baked background a human glances at before a dark slide).*
- ***`og: { src, w, h }`*** *— wide cover; a **declared** `og:image` verified at **≥600px actual width** (the declared meta size lies).*

*Measured by `scripts/fc.py logos` (`sips` for raster px; an SVG's box parsed from its text — stdlib, **no image dep**). Rationale + the 3-probe evidence: [`_design/2026-06-03-logos.md`](_design/2026-06-03-logos.md).*

*<a id="logos-lint"></a>**Lint — the measurement is the fact, so it's enforced.** When a `logos:{}` block is present, `fc.py verify`'s step-7 profile lint requires each recorded slot to carry its measurements — `wordmark`/`og`: `w`+`h`; `logomark`: `px`+`transparent`. A slot may be **omitted** (true absence), but a mark recorded **without** its measurement fails the lint: the consumer gates on the size, so a sizeless mark is decoration, not state. (The bakeoff shipped 7 URL-wordmarks with no `w`/`h` before this gate — agents measured the extracted `assets/wordmark.svg` but skipped measuring a hostable-URL wordmark.)*
...
- *`alternateName` → **`aliases`**; `legalName` → **`legal_entity`** (2.6 — its own queryable scalar, e.g. "Time Therapeutics, Inc." for Hone Health; `alternateName` stays an alias). Site-derivable only — empty when the page doesn't carry it, never back-filled by external lookup.*
...
| **Provenance** | A fixed, greppable set — one line each: **Pages** (analyzed + method) · **Verify** (sourceURL + md5 result) · **Credits** spent · **Couldn't get** (what + why). Optionally **Run profile** — only when a **guided** run was non-vanilla: what an emphasis / added module / forced-refresh-over-warm changed vs. a plain capture (e.g. `Run profile: guided — emphasis "enterprise pricing"; +offerings`); absent = plain run, see skill step 2.5. Optionally **Enriched** (model knowledge) — *only* the rare identity prior taken from the model, not the page (e.g. `Enriched (model knowledge): ETSY ticker; Reverb→reverb.com`); distinct from `unverified_fields` ("couldn't get it"), this is "got it — from the model, not the site." Optionally **Migrations** — added only to a profile a later migration *rule-rewrote* (not re-captured), one line per migration: date, version delta, value before→after (see [Migrations](#migrations)). |
...
*Contracts live in `modules/` — the depth modules (`OFFERINGS.md`, `VISUAL.md`) sit flat; the cohort packs live in `modules/cohort-packs/`. That subdir is named **`cohort-packs/`, deliberately not `cohorts/`** — `cohorts/` is reserved for the architecture's store-level `cohorts/<category-slug>/` cross-company *signals* layer ([architecture](_design/2026-05-30-architecture.md)), a different concept (Signals by category, not per-domain State). `modules/` also holds **`RUNS.md`** — the lone non-State member: capture-run telemetry (which tool/model/effort produced a capture), contract at [`RUNS.md`](modules/RUNS.md).*

**Depth modules:**

- **`offerings.md`** — the per-SKU/molecule roster, for a cohort whose consumer needs the grain a family line collapses (a brand's six GLP-1 SKUs into one bullet). Opt-in; **first enabled set: the telehealth cohort**, default elsewhere is *don't write the file*. **Full contract → [`OFFERINGS.md`](modules/OFFERINGS.md)** (roster-first; molecule rides `What` page-attested; lint `scripts/offeringscheck.py`; rationale [`_design/2026-06-03-offerings.md`](_design/2026-06-03-offerings.md)). When active it owns `price_visibility` per-SKU; `profile.md`'s family-line token stays the convention everywhere else.
- **`visual.md`** — the per-company **visual-evidence layer**: blind, cited cards across typography / layout / color-brand-imagery / iconography + a ~5-second `Visual & brand impression`, mined from screenshots (the visual-*execution* sibling to `brand.md`'s voice/positioning). Opt-in, own `captured_at`; written by the `/visual-evidence` skill *after* capture (blinding), never inside it. **Full contract → [`VISUAL.md`](modules/VISUAL.md)** (cards-first; **no score** — lint `scripts/visualcheck.py` fails on any `score:` field; rationale [`FRAME.md`](experiments/2026-06-13-visual-quality-graduation-frame/FRAME.md)).
...
- **`telehealth.md`** — per-company telehealth classification: **8 single-select cuts** (`value_chain_role`, `pharmacy_model`, `audience`, `compounding_posture`, `anchor_category`, `modality`, `access_model`, `pay_model`) that the universal profile reads identical for every telehealth brand, plus a page-attested body (the pharmacy-ownership claim verbatim, categories served, health-merchant credibility). Opt-in; **first enabled set: the telehealth cohort**. **Full contract → [`TELEHEALTH.md`](modules/cohort-packs/TELEHEALTH.md)** (frontmatter-first closed sets; lint `scripts/cohortcheck.py --cohort telehealth`). Page-attested, never adjudicated — the ownership *claim* is recorded, never verified.
- **`productivity_saas.md`** — per-company classification for **horizontal productivity / work software** (docs, databases, project tools, meetings, chat, design, CRM-for-teams): **6 single-select cuts** (`primary_job`, `primary_user`, `ai_front_door`, `pricing_model`, `free_entry`, `platform_posture`) that the universal profile reads identical across the cohort, plus a page-attested body (platform/integrations claim, surfaces, AI capabilities, pricing, trust seals). Opt-in. **Full contract → [`PRODUCTIVITY_SAAS.md`](modules/cohort-packs/PRODUCTIVITY_SAAS.md)** (frontmatter-first closed sets; lint `scripts/cohortcheck.py --cohort productivity_saas`). Page-attested, never adjudicated. Hardened across a 2-paper + 1-live (5-capture) stress-test — see [`experiments/2026-06-16-productivity-saas-derisk/`](experiments/2026-06-16-productivity-saas-derisk/FINDINGS.md).
