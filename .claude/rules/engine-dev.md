...
- **Engine owns State — not Signals or Judgments.** What a company/offering *is* now (a snapshot): yes. **Signals** (funding/traffic/headcount on a time axis; news/M&A as dated events — the `competitive-traction` sibling) and **Judgments** (relevance/threat/fit, relative to the asker — the consumer): no. See the [frame](_design/2026-05-29-frame.md)'s three-kinds split.
