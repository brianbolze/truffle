...
- **Match-free.** Tools emit raw signal; matching results to a cohort/brand list is a separate composable step (the planned `_match.py`), never baked into a capture tool. Keeps every tool generic — `serpapi` does this; the legacy scripts that baked matching in are what we're undoing.
